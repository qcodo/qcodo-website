<?php
	...
	class CounterGen extends QBaseClass {
		...
		/**
		 * Protected member variable that maps to the database column counter.filename
		...
		/**
		 * Protected member variable that maps to the database column counter.token
		 * @var string strToken
		 */
		protected $strToken;
		...
		/**
		 * Protected member variable that maps to the database column counter.counter
		...
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			// Instantiate a new Counter object and return it

			// Do we have to expand anything?
			if ($objQueryBuilder->ExpandAsArrayNodes) {
				$objToReturn = array();
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Counter::InstantiateDbRow($objDbRow, null, $objQueryBuilder->ExpandAsArrayNodes, $objToReturn, $objQueryBuilder->ColumnAliasArray);
...
/*		public static function QueryArrayCached($strConditions, $mixParameterArray = null) {
			...
			$objCache = new QCache('query', 'counter_' . serialize($strConditions));
			if (!($strQuery = $objCache->GetData())) {
				// Not Found -- Go ahead and Create/Build out a new QueryBuilder object with Counter-specific fields
				$objQueryBuilder = new QQueryBuilder($objDatabase);
				Counter::GetSelectFields($objQueryBuilder);
				Counter::GetFromFields($objQueryBuilder);
		...
		*/
		public static function LoadByToken($strToken) {
			return Counter::QuerySingle(
				QQ::Equal(QQN::Counter()->Token, $strToken)
			);
		}
		...
		public static function GetJournalForId($intId) {
			...
			$objResult = $objDatabase->Query('SELECT * FROM counter WHERE id = ' .
		...
		public function GetJournal() {
			return Counter::GetJournalForId($this->intId);
		}
