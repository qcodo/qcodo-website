<?php
	 ...
	 * listing a collection of Counter objects.  It includes
	 ...
	 * 
	 */
	class CounterDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Counter datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Counter object, or it can be any QQNode extending from QQN::Counter().
		 * 
		 ...
		 * sorting using two fields from the Counter object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Counter
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Counter::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Counter, given the clauses above
			$this->DataSource = Counter::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Counter-based QQNode.
		 * It will also verify that it is a proper Counter-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'counter') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "counter".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Counter()->Id;
				case 'Filename': return QQN::Counter()->Filename;
				case 'Token': return QQN::Counter()->Token;
				case 'Counter': return QQN::Counter()->Counter;
				default: throw new QCallerException('Simple Property not found in CounterDataGrid content: ' . $mixContent);
