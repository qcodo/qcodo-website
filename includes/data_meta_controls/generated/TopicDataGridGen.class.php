<?php
	 ...
	 * listing a collection of Topic objects.  It includes
	 ...
	 * 
	 */
	class TopicDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Topic datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Topic object, or it can be any QQNode extending from QQN::Topic().
		 * 
		 ...
		 * sorting using two fields from the Topic object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Topic
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Topic::QueryCount($objCondition, $objClauses);
			...
			$this->DataSource = Topic::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Topic-based QQNode.
		 * It will also verify that it is a proper Topic-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'topic') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "topic".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Topic()->Id;
				case 'TopicLinkId': return QQN::Topic()->TopicLinkId;
				case 'TopicLink': return QQN::Topic()->TopicLink;
				case 'Name': return QQN::Topic()->Name;
