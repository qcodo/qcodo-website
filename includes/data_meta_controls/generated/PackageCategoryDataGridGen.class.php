<?php
	 ...
	 * listing a collection of PackageCategory objects.  It includes
	 ...
	 * 
	 */
	class PackageCategoryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this PackageCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the PackageCategory object, or it can be any QQNode extending from QQN::PackageCategory().
		 * 
		 ...
		 * sorting using two fields from the PackageCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from PackageCategory
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = PackageCategory::QueryCount($objCondition, $objClauses);
			...
			$this->DataSource = PackageCategory::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a PackageCategory-based QQNode.
		 * It will also verify that it is a proper PackageCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'package_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "package_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::PackageCategory()->Id;
				case 'ParentPackageCategoryId': return QQN::PackageCategory()->ParentPackageCategoryId;
				case 'ParentPackageCategory': return QQN::PackageCategory()->ParentPackageCategory;
				case 'Token': return QQN::PackageCategory()->Token;
				case 'OrderNumber': return QQN::PackageCategory()->OrderNumber;
				case 'Name': return QQN::PackageCategory()->Name;
				case 'Description': return QQN::PackageCategory()->Description;
				case 'PackageCount': return QQN::PackageCategory()->PackageCount;
				case 'LastPostDate': return QQN::PackageCategory()->LastPostDate;
				default: throw new QCallerException('Simple Property not found in PackageCategoryDataGrid content: ' . $mixContent);
