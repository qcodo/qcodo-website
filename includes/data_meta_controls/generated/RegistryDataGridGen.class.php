<?php
	 ...
	 * listing a collection of Registry objects.  It includes
	 ...
	 * 
	 */
	class RegistryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Registry datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Registry object, or it can be any QQNode extending from QQN::Registry().
		 * 
		 ...
		 * sorting using two fields from the Registry object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Registry
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Registry::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Registry, given the clauses above
			$this->DataSource = Registry::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Registry-based QQNode.
		 * It will also verify that it is a proper Registry-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'registry') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "registry".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Registry()->Id;
				case 'Name': return QQN::Registry()->Name;
				case 'Value': return QQN::Registry()->Value;
				default: throw new QCallerException('Simple Property not found in RegistryDataGrid content: ' . $mixContent);
