<?php
	 ...
	 * listing a collection of LoginTicket objects.  It includes
	 ...
	 * 
	 */
	class LoginTicketDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this LoginTicket datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the LoginTicket object, or it can be any QQNode extending from QQN::LoginTicket().
		 * 
		 ...
		 * sorting using two fields from the LoginTicket object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from LoginTicket
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = LoginTicket::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from LoginTicket, given the clauses above
			$this->DataSource = LoginTicket::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a LoginTicket-based QQNode.
		 * It will also verify that it is a proper LoginTicket-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'login_ticket') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "login_ticket".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::LoginTicket()->Id;
				case 'PersonId': return QQN::LoginTicket()->PersonId;
				case 'Person': return QQN::LoginTicket()->Person;
				default: throw new QCallerException('Simple Property not found in LoginTicketDataGrid content: ' . $mixContent);
