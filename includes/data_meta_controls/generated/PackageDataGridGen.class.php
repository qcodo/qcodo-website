<?php
	 ...
	 * listing a collection of Package objects.  It includes
	 ...
	 * 
	 */
	class PackageDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Package datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Package object, or it can be any QQNode extending from QQN::Package().
		 * 
		 ...
		 * sorting using two fields from the Package object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Package
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Package::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Package, given the clauses above
			$this->DataSource = Package::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Package-based QQNode.
		 * It will also verify that it is a proper Package-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'package') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "package".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Package()->Id;
				case 'PackageCategoryId': return QQN::Package()->PackageCategoryId;
				case 'PackageCategory': return QQN::Package()->PackageCategory;
				case 'Token': return QQN::Package()->Token;
				case 'Name': return QQN::Package()->Name;
				case 'Description': return QQN::Package()->Description;
				case 'LastPostDate': return QQN::Package()->LastPostDate;
				case 'LastPostedByPersonId': return QQN::Package()->LastPostedByPersonId;
				case 'LastPostedByPerson': return QQN::Package()->LastPostedByPerson;
				case 'TopicLink': return QQN::Package()->TopicLink;

				default: throw new QCallerException('Simple Property not found in PackageDataGrid content: ' . $mixContent);
