<?php
	 ...
	 * listing a collection of IssueField objects.  It includes
	 ...
	 * 
	 */
	class IssueFieldDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this IssueField datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the IssueField object, or it can be any QQNode extending from QQN::IssueField().
		 * 
		 ...
		 * sorting using two fields from the IssueField object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from IssueField
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = IssueField::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from IssueField, given the clauses above
			$this->DataSource = IssueField::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a IssueField-based QQNode.
		 * It will also verify that it is a proper IssueField-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'issue_field') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "issue_field".');
				...
				case 'ActiveFlag': return QQN::IssueField()->ActiveFlag;
				default: throw new QCallerException('Simple Property not found in IssueFieldDataGrid content: ' . $mixContent);
