<?php
	 ...
	 * listing a collection of ArticleSection objects.  It includes
	 ...
	 * 
	 */
	class ArticleSectionDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this ArticleSection datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the ArticleSection object, or it can be any QQNode extending from QQN::ArticleSection().
		 * 
		 ...
		 * sorting using two fields from the ArticleSection object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from ArticleSection
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = ArticleSection::CountAll();
			}
			...
			// Set the DataSource to be a Query result from ArticleSection, given the clauses above
			$this->DataSource = ArticleSection::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a ArticleSection-based QQNode.
		 * It will also verify that it is a proper ArticleSection-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'article_section') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "article_section".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::ArticleSection()->Id;
				case 'Name': return QQN::ArticleSection()->Name;
				default: throw new QCallerException('Simple Property not found in ArticleSectionDataGrid content: ' . $mixContent);
