<?php
	 ...
	 * listing a collection of Article objects.  It includes
	 ...
	 * 
	 */
	class ArticleDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Article datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Article object, or it can be any QQNode extending from QQN::Article().
		 * 
		 ...
		 * sorting using two fields from the Article object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Article
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Article::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Article, given the clauses above
			$this->DataSource = Article::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Article-based QQNode.
		 * It will also verify that it is a proper Article-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'article') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "article".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Article()->Id;
				case 'ArticleSectionId': return QQN::Article()->ArticleSectionId;
				case 'ArticleSection': return QQN::Article()->ArticleSection;
				case 'Title': return QQN::Article()->Title;
				case 'Description': return QQN::Article()->Description;
				case 'Byline': return QQN::Article()->Byline;
				case 'Article': return QQN::Article()->Article;
				case 'PostDate': return QQN::Article()->PostDate;
				case 'LastUpdatedDate': return QQN::Article()->LastUpdatedDate;
				default: throw new QCallerException('Simple Property not found in ArticleDataGrid content: ' . $mixContent);
