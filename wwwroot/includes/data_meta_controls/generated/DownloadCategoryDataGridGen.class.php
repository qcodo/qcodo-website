<?php
	 ...
	 * listing a collection of DownloadCategory objects.  It includes
	 ...
	 * 
	 */
	class DownloadCategoryDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this DownloadCategory datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the DownloadCategory object, or it can be any QQNode extending from QQN::DownloadCategory().
		 * 
		 ...
		 * sorting using two fields from the DownloadCategory object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from DownloadCategory
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = DownloadCategory::CountAll();
			}
			...
			// Set the DataSource to be a Query result from DownloadCategory, given the clauses above
			$this->DataSource = DownloadCategory::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a DownloadCategory-based QQNode.
		 * It will also verify that it is a proper DownloadCategory-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'download_category') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "download_category".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::DownloadCategory()->Id;
				case 'OrderNumber': return QQN::DownloadCategory()->OrderNumber;
				case 'Name': return QQN::DownloadCategory()->Name;
				case 'AnnounceOnlyFlag': return QQN::DownloadCategory()->AnnounceOnlyFlag;
				case 'Description': return QQN::DownloadCategory()->Description;
				case 'Note': return QQN::DownloadCategory()->Note;
				case 'LastPostDate': return QQN::DownloadCategory()->LastPostDate;
				default: throw new QCallerException('Simple Property not found in DownloadCategoryDataGrid content: ' . $mixContent);
