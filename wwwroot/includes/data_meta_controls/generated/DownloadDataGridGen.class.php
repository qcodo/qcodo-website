<?php
	 ...
	 * listing a collection of Download objects.  It includes
	 ...
	 * 
	 */
	class DownloadDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Download datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Download object, or it can be any QQNode extending from QQN::Download().
		 * 
		 ...
		 * sorting using two fields from the Download object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Download
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Download::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Download, given the clauses above
			$this->DataSource = Download::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Download-based QQNode.
		 * It will also verify that it is a proper Download-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'download') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "download".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Download()->Id;
				case 'ParentDownloadId': return QQN::Download()->ParentDownloadId;
				case 'ParentDownload': return QQN::Download()->ParentDownload;
				case 'DownloadCategoryId': return QQN::Download()->DownloadCategoryId;
				case 'DownloadCategory': return QQN::Download()->DownloadCategory;
				case 'PersonId': return QQN::Download()->PersonId;
				case 'Person': return QQN::Download()->Person;
				case 'Name': return QQN::Download()->Name;
				case 'Version': return QQN::Download()->Version;
				case 'Description': return QQN::Download()->Description;
				case 'Filename': return QQN::Download()->Filename;
				case 'DownloadCount': return QQN::Download()->DownloadCount;
				case 'PostDate': return QQN::Download()->PostDate;
				default: throw new QCallerException('Simple Property not found in DownloadDataGrid content: ' . $mixContent);
