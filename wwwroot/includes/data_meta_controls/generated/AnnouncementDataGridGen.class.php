<?php
	 ...
	 * listing a collection of Announcement objects.  It includes
	 ...
	 * 
	 */
	class AnnouncementDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Announcement datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Announcement object, or it can be any QQNode extending from QQN::Announcement().
		 * 
		 ...
		 * sorting using two fields from the Announcement object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Announcement
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = Announcement::CountAll();
			}
			...
			// Set the DataSource to be a Query result from Announcement, given the clauses above
			$this->DataSource = Announcement::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Announcement-based QQNode.
		 * It will also verify that it is a proper Announcement-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'announcement') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "announcement".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Announcement()->Id;
				case 'Announcement': return QQN::Announcement()->Announcement;
				default: throw new QCallerException('Simple Property not found in AnnouncementDataGrid content: ' . $mixContent);
