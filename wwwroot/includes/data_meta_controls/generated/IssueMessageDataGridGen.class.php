<?php
	 ...
	 * listing a collection of IssueMessage objects.  It includes
	 ...
	 * 
	 */
	class IssueMessageDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this IssueMessage datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the IssueMessage object, or it can be any QQNode extending from QQN::IssueMessage().
		 * 
		 ...
		 * sorting using two fields from the IssueMessage object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from IssueMessage
		...
		public function MetaDataBinder() {
			...
			if ($this->Paginator) {
				$this->TotalItemCount = IssueMessage::CountAll();
			}
			...
			// Set the DataSource to be a Query result from IssueMessage, given the clauses above
			$this->DataSource = IssueMessage::LoadAll($objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a IssueMessage-based QQNode.
		 * It will also verify that it is a proper IssueMessage-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'issue_message') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "issue_message".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::IssueMessage()->Id;
				case 'IssueId': return QQN::IssueMessage()->IssueId;
				case 'Issue': return QQN::IssueMessage()->Issue;
				case 'PersonId': return QQN::IssueMessage()->PersonId;
				case 'Person': return QQN::IssueMessage()->Person;
				case 'Message': return QQN::IssueMessage()->Message;
				case 'ReplyNumber': return QQN::IssueMessage()->ReplyNumber;
				case 'PostDate': return QQN::IssueMessage()->PostDate;
				default: throw new QCallerException('Simple Property not found in IssueMessageDataGrid content: ' . $mixContent);
